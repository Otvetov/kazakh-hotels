...
<div class="max-w-6xl mx-auto px-4 py-8">
    <div class="mb-6">
        <h1 class="text-white text-2xl md:text-3xl font-extrabold mb-1">Оформление бронирования</h1>
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="otl-surface overflow-hidden">
                <div class="p-6 border-b border-white/10">
                    <h2 class="text-white text-xl font-bold">Детали бронирования</h2>
                </div>
                <div class="p-6 space-y-6">
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            <div class="text-sm text-[#7e8488] mb-1">Отель</div>
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            <div class="text-sm text-[#7e8488] mb-1">Номер</div>
                            <div class="text-white font-medium">{{ $room->name }}</div>
                            <div class="text-sm text-[#7e8488] mt-1">Вместимость: до {{ $room->capacity }} {{ $room->capacity == 1 ? 'гостя' : 'гостей' }}</div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            <div class="text-sm text-[#7e8488] mb-2">Даты проживания</div>
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <div class="text-xs text-[#7e8488] mb-1">Заезд</div>
                                    <div class="text-white font-medium" id="check-in-display">
                                        {{ $checkIn ? \Carbon\Carbon::parse($checkIn)->format('d.m.Y') : 'Не выбрано' }}
                                    </div>
                                </div>
                                <div>
                                    <div class="text-xs text-[#7e8488] mb-1">Выезд</div>
                                    <div class="text-white font-medium" id="check-out-display">
                                        {{ $checkOut ? \Carbon\Carbon::parse($checkOut)->format('d.m.Y') : 'Не выбрано' }}
                                    </div>
                                </div>
                            </div>
                            <div class="mt-2 text-sm text-[#7e8488]">Всего ночей: <span id="nights-count">{{ $nights }}</span></div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="otl-surface overflow-hidden">
                <div class="p-6 border-b border-white/10">
                    <h2 class="text-white text-xl font-bold">Данные гостей</h2>
                </div>
                <div class="p-6">
                    <form id="booking-form" action="{{ route('bookings.store') }}" method="POST">
                        ...
                        <div class="space-y-5">
                            <div>
                                <label class="block text-sm text-[#7e8488] mb-2">Количество гостей</label>
                                <input type="number" name="guests" required min="1" max="{{ $room->capacity }}"
                                       value="{{ old('guests', $guests) }}" class="field-input">
                            </div>
                            <div>
                                <label class="block text-sm text-[#7e8488] mb-2">Особые пожелания <span class="text-[#7e8488]">(опционально)</span></label>
                                <textarea name="special_requests" rows="4"
                                          placeholder="Ранний заезд, поздний выезд, дополнительная подушка и т.д."
            ...
            <div class="sticky top-24">
                <div class="otl-surface overflow-hidden">
                    <div class="p-6 bg-gradient-to-br from-[#8ee30f] to-[#76c406]">
                        <h2 class="text-[#0a0a0a] text-lg font-bold mb-1">Итоговая стоимость</h2>
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div class="bg-[#141516] rounded-2xl p-4 space-y-2">
                            @foreach(['Бесплатная отмена за 24 часа', 'Мгновенное подтверждение', 'Безопасная оплата'] as $perk)
                        ...
                        <p class="text-xs text-[#7e8488] text-center">
                            Нажимая «Забронировать», вы соглашаетесь с условиями бронирования и политикой конфиденциальности
                        </p>
                    </div>
                </div>
            </div>
    ...
    function calculatePrice() {
        ...
        if (checkIn && checkOut) {
            ...
            if (nights > 0) {
                ...
                checkInDisplay.textContent = checkInDate.toLocaleDateString('ru-RU', opt);
                checkOutDisplay.textContent = checkOutDate.toLocaleDateString('ru-RU', opt);
            }
        }
    }
